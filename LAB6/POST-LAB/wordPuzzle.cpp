 ...
 */
int main(int argc, char* argv[]) {  //LOOK OVER
    
    // to hold the number of rows and cols in the input file
    int rows, cols;
    int num_found_words = 0;
    string outp;
    string dir;
    string dir2;
    timer t;
    ...
    //bool result2 = readInDict(argv[1]);
    ...
    //cout << h->find("nab") << endl;
    for (int r = 0; r < rows; r++) {
        for (int c = 0; c < cols; c++) {
            for (int d = 0; d < 8; d++) { //given
                for (int l = 3; l <= h->max_size; l++) {
                   string letters = getWordInGrid(r, c, d, l, rows, cols);
                   if (letters.length() == l) {
                       if(h->find(letters))  {
                           //if(prefix->find(letters)) {
                               //bool found = h->find(letters);
                           bool b = true;
                           switch (d) { //given
                                 case 0:
                                   dir = "N";
...
//                                     if (letters.length() >= 3) {
//                                         if (found) {
//                                             output = dir + " (" + to_string(r) + ", " + to_string(c) + "):\t" + letters;
//                                     }
                                    if (b) {
                                    output = dir + " (" + to_string(r) + ", " + to_string(c) + "):\t" + letters;
                                    }
                                    else {
                                    output = dir2 + "(" + to_string(r) + ", " + to_string(c) + "):\t" + letters;
                                    }
                                    outp = outp + output + "\n";
                                    num_found_words++;
                            }
    ...
    cout << outp;
    t.stop();
    cout << num_found_words << " words found" << endl;
