...
heap heapTree::createHuffTree(heap min) {
    while(min.size() > 1) { // while there is more than 1 node present in heap
        heapNode *h1 = min.deleteMin();
        heapNode *h2 = min.deleteMin();
        int total_freq = 0; //initialize count to 0
        total_freq += (h1 -> getFre()); //get frequency/amount of times h1 node is present in heap
        total_freq += (h2 -> getFre()); //get frequency/amount of times h2 node is present in heap
        heapNode* total = new heapNode(total_freq, 'e'); // 'e' is place holder
        total->left = h1; //change left pointer to make child of new node
        total->right = h2; //change right pointer to make child of new node
        min.insert(total); //insert new tree node into heap
    }
    return min;
}
